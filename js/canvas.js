export function initCanvas(canvasElement) {
    const rectContainsPoint = (rect, point) => point.x >= rect.x && point.x <= (rect.x + rect.w) && point.y >= rect.y && point.y <= (rect.y + rect.h);
    /**
     * @returns A rect surrounding all the rects.
     */
    const rectMerge = (rects) => {
    ...
    class Control {
        constructor(ctx) {
            this.ctx = ctx;
            this.handleEvent = _ => this;
            this.draw = () => { };
            this.measureRect = () => ({ x: 0, y: 0, h: 0, w: 0 });
        }
    }
    ...
    class Contextmenu extends Control {
        constructor(ctx, x, y, onDismiss, items) {
            ...
            this.measureRect = () => rectMerge(this.items.map(item => item.measureRect()));
            const itemWidth = 150;
            const itemHeight = 20;
    ...
    class Canvas2d {
        constructor(canvasElement) {
                ...
                if (e.target === this.canvasElement) {
                    ...
                    if (e instanceof MouseEvent) {
                        ...
                        if (e.type === "contextmenu") {
                            e.preventDefault();
                            const contextMenu = new Contextmenu(this.ctx, e.offsetX, e.offsetY, () => {
            ...
            this.dispatchEventToControls = (event) => {
                const newControls = [];
